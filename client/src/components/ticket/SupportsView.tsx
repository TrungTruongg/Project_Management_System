...
function SupportsView() {
  ...
  const handleUpdateTicket = (updatedTicket: any) => {
    setSupTickets(
      supTickets.map((ticket: any) => (ticket._id === updatedTicket._id ? updatedTicket : ticket))
    );
  };
  ...
  const handleDeleteTicket = async () => {
    ...
    try {
      ...
      setSupTickets(supTickets.filter((ticket: any) => ticket._id !== selectedTicket._id));
  ...
  const checkCompleteSupport = async (ticket: any) => {
    try {
      ...
      await api.put(`/tickets/update/${ticket._id}`, updatedTicket);

      setSupTickets(supTickets.map((t: any) => (t._id === ticket._id ? updatedTicket : t)));
    } catch (error) {
  ...
  const isTaskLeaderForTicket = (ticket: any): boolean => {
    if (!ticket.taskId || !user) return false;
    const task = tasks.find((t: any) => t._id === ticket.taskId);
    return task && task.leaderId === user._id;
  };
  ...
  return (
    <>
      ...
      {loading ? (
        ...
        <Typography fontStyle="italic">You need have task to create support ticket.</Typography>
      ) : supTickets.length === 0 ? (
        ...
        >
          <Table>
            <TableHead>
              <TableRow
                sx={{ bgcolor: (theme) => (theme.palette.mode === 'light' ? '#f5f5f5' : 'black') }}
              >
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell align="center" onClick={(e) => e.stopPropagation()}>
                        {isTaskLeaderForTicket(ticket) && ticket.status !== 'completed' && (
                          <IconButton
                            size="small"
                            sx={{ color: '#4CAF50' }}
                            onClick={(e) => {
