...
function Projects() {
  ...
  const fetchAllData = async () => {
    setLoading(true);
    try {
      const [responseProject, responseTask, responseUser] = await Promise.all([
        api.get("/projects"),
        api.get("/tasks"),
        api.get("/users"),
      ]);
  ...
  return (
    <>
      ...
      {loading ? (
        ...
        >
          {filteredProjects.map((project: any) => {
            ...
            const completion = calculateProjectCompletion(project._id);
            const projectLeader = users.find((user) => user._id === project.leaderId);
            ...
            return (
              <Card
                key={project._id}
                elevation={0}
                onClick={() => handleViewTasksInProject(project._id)}
                sx={{
                  border: (theme) =>
                      ...
                      <Chip
                        label={
                          isProjectExpired(project.endDate)
                            ? "Expired"
                            : (project.endDate && project.startDate) === "" ? "No deadline"
                              : `${calculateDeadline(
                                project.startDate,
                                project.endDate
                              )} Days Left`
      ...
      <DeleteConfirmDialog
        ...
        onDelete={() => handleDeleteProject(selectedProject ? selectedProject._id : "")}
