...
function AttachmentPreview({ attachment, onDelete }: { attachment: AttachmentItem; onDelete: () => void }) {
  const [mediaType, setMediaType] = useState<
    'image' | 'video' | 'document' | 'excel' | 'word' | 'powerpoint' | 'none'
  >('none');
    ...
    <>
      <Box
        sx={{
          ...
          position: 'relative',
          bgcolor: ['document', 'excel', 'word', 'powerpoint'].includes(mediaType) ? '#f5f5f5' : 'white',
          overflow: 'hidden',
      ...
      >
              ...
              if (['doc', 'docx', 'xls', 'xlsx', 'ppt', 'pptx'].includes(ext || '')) {
                return (
                  ...
                  >
                    ...
                    <Typography
                      variant="body2"
                      sx={{ fontWeight: 500, textAlign: 'center', px: 2 }}
                    >
                      {attachment.name}
                    </Typography>
                  </Box>
        ...
        <PreviewActionButtons attachment={{ ...attachment, url: attachment.url }} onDelete={onDelete} />
      </Box>
          ...
          {!loading && (
            ...
            >
              {mediaType === 'image' ? (
                ...
                <iframe
                  src={attachment.name?.endsWith('.pdf') ? `${attachment.url}#toolbar=0` : attachment.url}
                  style={{
                    width: '100%',
                    height: '100%',
                    border: 'none',
